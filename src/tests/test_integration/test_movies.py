...
from database import ActorModel, CountryModel, GenreModel, LanguageModel, MovieModel
...
async def test_get_movie_by_id_fields_match_database(client, db_session, seed_database):
    ...
    assert response_data["date"] == random_movie.date.isoformat(), (
        "Date does not match."
    )
    assert response_data["score"] == random_movie.score, "Score does not match."
    assert response_data["overview"] == random_movie.overview, (
        "Overview does not match."
    )
    assert response_data["status"] == random_movie.status.value, (
        "Status does not match."
    )
    assert response_data["budget"] == float(random_movie.budget), (
        "Budget does not match."
    )
    assert response_data["revenue"] == random_movie.revenue, "Revenue does not match."

    assert response_data["country"]["id"] == random_movie.country.id, (
        "Country ID does not match."
    )
    assert response_data["country"]["code"] == random_movie.country.code, (
        "Country code does not match."
    )
    assert response_data["country"]["name"] == random_movie.country.name, (
...
async def test_create_movie_and_related_models(client, db_session):
    ...
    assert response_data["overview"] == movie_data["overview"], (
        "Movie overview does not match."
    )
...
async def test_create_movie_duplicate_error(client, db_session, seed_database):
    ...
    response_data = response.json()
    expected_detail = f"A movie with the name '{movie_data['name']}' and release date '{movie_data['date']}' already exists."
